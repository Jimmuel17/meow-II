...
@section('content')
    <div class="row">
        <div class="col-12 grid-margin">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Edit School Year</h4>
                    <form class="forms-sample" method="POST" enctype="multipart/form-data" action="{{route('schoolyear.update',['schoolyear'=>$item->id])}}">
                        @csrf
                        @method('PUT')
                        <div class="form-group row">
                            <label for="exampleInputUsername2" class="col-sm-3 col-form-label">Start Year</label>
                            <div class="col-sm-9">
                                <input type="text" name="startyear" class="form-control" id="startyear" value="{{$item->startyear}}" placeholder="Enter Start School Year">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="exampleInputUsername2" class="col-sm-3 col-form-label">End Year</label>
                            <div class="col-sm-9">
                                <input type="text" name="endyear" class="form-control" id="endyear" value="{{$item->endyear}}" placeholder="Enter End School Year">
                            </div>
                        </div>
                        <div class="form-group row">
